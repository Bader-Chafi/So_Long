...
void	initialise_struct(t_obj *game)
{
	...
	game->mlx = mlx_init();
	if (!game->mlx)
		ft_puterror(game, "Error: mlx_init() failed !!");
	game->win = mlx_new_window(game->mlx, game->width, game->height,
			"SO LONG ** Take all Collect then exit from the door **");
	game->floor = mlx_xpm_file_to_image(game->mlx, "M/XPM/back.xpm", &s, &s);
	game->wall[0] = mlx_xpm_file_to_image(game->mlx, "M/XPM/wallP.xpm", &s, &s);
	game->wall[1] = mlx_xpm_file_to_image(game->mlx, "M/XPM/wall.xpm", &s, &s);
	game->candy = mlx_xpm_file_to_image(game->mlx, "M/XPM/candy.xpm", &s, &s);
	game->choper[0] = mlx_xpm_file_to_image(game->mlx, "M/XPM/ple.xpm", &s, &s);
	game->choper[1] = mlx_xpm_file_to_image(game->mlx, "M/XPM/pri.xpm", &s, &s);
